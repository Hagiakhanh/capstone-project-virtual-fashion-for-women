         ...
         <div className="bg-white p-10 rounded-2xl" style={{ boxShadow: '0px 0px 24px rgba(0, 0, 0, 0.1)' }}>
            <div className="flex uppercase justify-center gap-20 text-2xl font-normal text-gray-600 mb-5">
               ...
               <h1 onClick={() => setActiveTab(3)} className={`relative group cursor-pointer ${activeTab === 3 ? 'font-bold text-black' : ''}`}>
                  Bán chạy nhất
                  <span className={`${activeTab === 3 ? 'w-full' : ''} absolute -bottom-1.5 left-0 h-[3px] w-0 bg-black transition-all duration-200 ease-out group-hover:w-full`}></span>
               </h1>
               <h1 onClick={() => setActiveTab(4)} className={`relative group cursor-pointer ${activeTab === 4 ? 'font-bold text-black' : ''}`}>
                  Đang giảm giá
                  <span className={`${activeTab === 4 ? 'w-full' : ''} absolute -bottom-1.5 left-0 h-[3px] w-0 bg-black transition-all duration-200 ease-out group-hover:w-full`}></span>
               </h1>
            </div>
